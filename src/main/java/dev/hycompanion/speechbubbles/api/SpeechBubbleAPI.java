...
/**
 * Public API for the Speech Bubbles plugin.
 * 
 * This class provides a simple interface for other plugins to display speech bubbles
 * above entities in the Hytale world. All methods are thread-safe and can be called
 * from any thread.
 * 
 * <p><b>Basic Usage:</b></p>
 * <pre>
 ...
 * 
 * // Show a bubble with custom duration
 * SpeechBubbleAPI.showBubble(npcUuid, playerUuid, "Welcome to my shop!", 8000);
 * 
 * // Show a bubble with full customization
 * SpeechBubbleOptions options = new SpeechBubbleOptions()
 *     .duration(10000)
 *     .maxWidth(300)
 *     .maxHeight(200)
 *     .textColor("#FFD700");
 * SpeechBubbleAPI.showBubble(npcUuid, playerUuid, "Check out my wares!", options);
 * </pre>
 * 
...
public final class SpeechBubbleAPI {
    ...
    public static boolean isAvailable() {
        return SpeechBubblesEntrypoint.getInstance() != null 
            && SpeechBubblesEntrypoint.getInstance().getBubbleManager() != null;
    }
    ...
    public static boolean showBubble(@Nonnull UUID entityUuid, @Nonnull UUID playerUuid, @Nonnull String text) {
        return showBubble(entityUuid, playerUuid, text, (SpeechBubbleOptions) null);
    }
    ...
    public static boolean showBubble(@Nonnull UUID entityUuid, @Nonnull UUID playerUuid, @Nonnull String text, long durationMs) {
        return showBubble(entityUuid, playerUuid, text, new SpeechBubbleOptions().duration(durationMs));
    }
     ...
     * @return true if the bubble was successfully queued for display
     */
    public static boolean showBubble(@Nonnull UUID entityUuid, @Nonnull UUID playerUuid, @Nonnull String text, @Nullable SpeechBubbleOptions options) {
        SpeechBubbleManager manager = getManager();
        if (manager == null) {
            return false;
        }
     ...
     */
    public static int showBubbleToAll(@Nonnull UUID entityUuid, @Nonnull String text, @Nullable SpeechBubbleOptions options) {
        SpeechBubbleManager manager = getManager();
        if (manager == null) {
            return 0;
        }
     ...
     */
    @Nullable
    private static SpeechBubbleManager getManager() {
        SpeechBubblesEntrypoint instance = SpeechBubblesEntrypoint.getInstance();
        if (instance == null) {
            return null;
        }
